...
class MyViewModel : ViewModel() {
    ...
    val injector = DataInjector(searcher) // #1: Create your injector
    val dataSourceFactory = SearcherSingleIndexDataSource.Factory(searcher) { hit ->
        ...
        injector.inject(it) // #2: inject data into your page list
    }
...
class DataInjector(
    ...
    var mapper: ((MutableList<Product>) -> List<Product>)? = null
) {

    init {
        searcher.response.subscribePast { response ->
            if (response == null) return@subscribePast
            val data = response.userDataOrNull
            mapper = { list -> // #3: Business logic here
                list.add(2, Product("AdWards: $data", null))
                list
            }
        }
    }

    fun inject(list: MutableList<Product>): List<Product> {
        return mapper?.invoke(list) ?: list
    }
}
